...
from official.wide_deep import multi_node_wide_deep_run_loop
...
def define_census_flags():
  multi_node_wide_deep_run_loop.define_wide_deep_flags()
  flags.adopt_module_key_flags(multi_node_wide_deep_run_loop)
...
def build_estimator(model_dir, model_type, model_column_fn, inter_op, intra_op):
  ...
  distribution_strategy = tf.contrib.distribute.ParameterServerStrategy(num_gpus_per_worker=4)
...
def run_census(flags_obj):
  ...
  def train_input_fn():
    ...
    #    train_file, flags_obj.epochs_between_evals, True, flags_obj.batch_size)
        train_file, 1000, True, flags_obj.batch_size)
  ...
  multi_node_wide_deep_run_loop.run_loop(
